...
2. **Per-call instrumentation logs.** A one-shot log at install time (`[INFO] patched N blocks with kernel X`, `[WARN] could not extract required attribute on a patched block`) tells you the patch attempt happened and which fork it took. Per-call logging is finer-grained but noisier and only useful when you suspect a subset of calls is misbehaving. **Every fallback path needs a log line, and every fallback log line must carry the underlying error's message (not just its class name).** Five distinct ways this rule gets violated, all functionally equivalent for debugging utility:
   ...
   **(f) The log line is emitted, correctly, and never reaches you.** All of (a)-(e) are about whether the line gets *written*. This one is about the transport, and it defeats the whole rung without touching the code under test. Redirecting a launcher to a file (`./start.sh > log 2>&1`) switches stdout from line-buffered to block-buffered, and a 4-8 KB boundary lands wherever it lands -- in the case that cost three restarts on 2026-08-06, mid model-load, so every engagement line sat in an unflushed buffer while the file showed a clean startup and nothing else. The observer's conclusion was "the patch silently failed to engage," which is indistinguishable from (a)-(c) and is the exact wrong conclusion. `PYTHONUNBUFFERED=1` does **not** reliably fix it: if the launcher goes through a wrapper (`uv run --active`), the variable does not propagate to the child. Use `stdbuf -oL -eL ./start.sh`, and prefer HTTP status endpoints (`/queue`, `/history`) over log scraping for run state, since those are never buffered.
